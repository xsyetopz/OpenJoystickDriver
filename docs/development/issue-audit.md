...
Audit baseline: 2026-07-19, OpenJoystickDriver 0.5 development tree.
...
| --- | --- | --- | --- | --- |
| [#8 Steam Controller](https://github.com/xsyetopz/OpenJoystickDriver/issues/8) | Wired and wireless devices enumerate without mouse/keyboard lizard behavior; buttons, sticks, pads, lifecycle, haptics, and brightness work through a consumer-visible virtual controller. | Catalog records, `SteamControllerParser`, feature-report handling, lifecycle gating, parser tests, and the macOS 14 parser harness pass. | **Partial: code implemented, hardware unverified.** | Run the [Steam Controller procedure](../testing/steam-controller.md) on wired and wireless hardware, including reconnect and consumer visibility. |
| [#9 Xbox 360 wireless](https://github.com/xsyetopz/OpenJoystickDriver/issues/9) | Receiver opens without a crash, detects controller connect/disconnect, maps every input correctly, and accepts supported output. | Wireless receiver records, wrapped-input parsing, lifecycle handling, output-packet tests, and SwiftUSB's device/context lifetime fix are present. | **Partial: code implemented, hardware unverified.** | Repeat the [wireless receiver procedure](../testing/xbox-360-wireless-receiver.md) with SwiftUSB 0.1.2; the last reported live probe crashed before acceptance. |
...
| [#11 Logitech F310](https://github.com/xsyetopz/OpenJoystickDriver/issues/11) | XInput-mode controls match SDL's mapping. | The generated record selects the Xbox 360 parser with interrupt endpoints `0x81`/`0x02`; mapping and transport-profile tests pass. | **Code-resolved, hardware unverified.** | Run the [F310 procedure](../testing/logitech-f310.md) and compare every control with SDL on the same Mac. |
| [#14 Wolverine V3 TE](https://github.com/xsyetopz/OpenJoystickDriver/issues/14) | `1532:0A43` uses GIP rather than Generic HID, completes startup, exposes every verified control, reconnects, and supports only observed output capabilities. | The record selects GIP with parser defaults. Unsupported Share and paddle claims were removed; provenance remains unverified. | **Partial: classification implemented.** | Capture endpoints, startup, every input, reconnect, indicator, and rumble using the [V3 TE procedure](../testing/razer/v3-te.md). |
...
#14 (1), #18 (4), and #19 (1). These filters prove the recorded parser,
