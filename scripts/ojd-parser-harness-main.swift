...
func runProfileMetadataChecks() {
  ...
  require(
    steamWiredProfile.mappingFlags == [
      "lizardMode", "trackpads", "experimental", "needsHardwareTest",
    ],
...
func runSteamInputAndFeatureChecks() throws {
  ...
  let leftPadOnlyEvents = try leftPadOnly.parse(data: makeSteamControllerReport(
    b10: 0x08,
    leftX: 32767,
    leftY: -32767
  ))
  ...
  require(
    hasEvent(leftPadOnlyEvents, .buttonPressed(.genericButton4)),
    "Steam left pad touch should emit touch button"
  )

  let leftPadAndJoy = SteamControllerParser()
  _ = try leftPadAndJoy.parse(data: makeSteamControllerReport())
  let leftPadAndJoyEvents = try leftPadAndJoy.parse(data: makeSteamControllerReport(
    b10: 0x88,
    leftX: 32767,
    leftY: -32767
  ))
  require(
    ...
    "Steam lpad+joy bit should allow left-stick motion"
  )
  require(
    hasEvent(leftPadAndJoyEvents, .buttonPressed(.genericButton4)),
    "Steam lpad+joy bit should emit touch button"
  )
  ...
  require(
    Array(startup[1].bytes.prefix(8)) == [0x87, 6, 0x07, 0x07, 0, 0x08, 0x07, 0],
    "Steam startup should disable trackpad mouse modes"
  )
...
func runDS3InputChecks() throws {
  ...
  for expected in [
    Button.back, .leftStick, .rightStick, .start, .l2Digital, .r2Digital, .l1, .r1,
...
func runDualSenseUSBChecks() throws {
  ...
  require(
    hasEvent(micEvents, .buttonPressed(.genericButton1)),
    "DualSense USB should parse mic mute"
  )
}
...
func runDualSenseBluetoothCRCCheck() throws {
  ...
  require(
    hasEvent(events, .buttonPressed(.genericButton1)),
...
func runSwitchProTransportAndMappingCheck() throws {
  ...
  require(
    parser.hidStartupReports(transport: "Bluetooth").isEmpty,
    "Switch Pro Bluetooth should skip USB startup reports"
  )
  require(
  ...
  require(
    parser.hidStartupReports(transport: "USB").map(\.reportID) == [0x80, 0x80, 0x80, 0x80, 0x01],
  ...
  for expected in [
    Button.a, .b, .x, .y, .leftBumper, .rightBumper, .l2Digital, .r2Digital, .back,
    .start, .leftStick, .rightStick, .guide, .share,
  ] {
    require(
      hasEvent(buttonEvents, .buttonPressed(expected)),
  ...
  let stickEvents = try stickParser.parse(data: makeSwitchProReport(
    leftX: 4095,
    leftY: 0,
    rightX: 0,
    rightY: 4095
