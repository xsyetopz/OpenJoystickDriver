...
- Added GameController diagnostics for Browser API verification, including
  synthetic user-space input injection and focused haptics checks.
- Added same-model controller disambiguation through serial/location-aware XPC
  queries, GUI identity keys, packet logs, and physical rumble targeting.
- Added daemon-owned Input Monitoring and Accessibility prompt flow coverage,
  including localized permission labels for the setup UI.

### Changed

- Changed Compatibility mode routing to keep one shared user-space virtual
  controller while neutralizing inactive routes, avoiding duplicate browser and
  consumer devices.
- Changed `generic-hid` and `sdl2-3` Compatibility identities to publish the
  Safari-accepted Xbox 360 HID surface so Browser Gamepad API pages enumerate
  and receive input.
- Updated backend diagnostics to fail when critical DriverKit, SDL, or
  GameController acceptance probes fail instead of masking failures.

### Fixed

- Fixed Safari Browser Gamepad API support for SDL2/3, Generic HID, Apple
  GameController, Xbox 360 HID, and Xbox One HID compatibility modes.
- Fixed Xbox One HID Browser API rumble parsing when Safari includes a
  duplicate output report ID byte in the payload.
- Fixed Xbox One HID Browser API center-button/stick-click ordering so View,
  Menu, L3, and R3 report through the expected Safari buttons.
- Fixed `x360-hid` Browser API enumeration by using the GameController-accepted
  Xbox 360 wired identity.
- Fixed stale GUI replacement during `rebuild-fast` by quitting the running app
  before replacing `/Applications/OpenJoystickDriver.app`.
- Fixed permission status reporting so Input Monitoring and Accessibility are
  checked and displayed separately for the daemon helper.
