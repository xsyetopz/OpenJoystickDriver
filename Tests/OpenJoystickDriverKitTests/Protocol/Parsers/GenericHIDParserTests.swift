...
struct GenericHIDParserTests {
  ...
  @Test func normalizesAndPairsStandardStickAxes() {
    ...
    #expect(
      parser.parse(elementValue: axis(usage: 0x30, integer: 255)) == [.leftStickChanged(x: 1, y: 0)]
    )
    #expect(
      parser.parse(elementValue: axis(usage: 0x31, integer: 0)) == [.leftStickChanged(x: 1, y: 1)]
    )
    #expect(
      parser.parse(elementValue: axis(usage: 0x33, integer: 0)) == [.rightStickChanged(x: -1, y: 0)]
    )
    #expect(
      parser.parse(elementValue: axis(usage: 0x34, integer: 255)) == [
  ...
  @Test func mapsTriggersHatAndIgnoresUnknownUsages() {
    ...
    #expect(
      parser.parse(elementValue: axis(usage: 0x32, integer: 128)) == [
        .leftTriggerChanged(Float(128) / 255)
      ]
    )
    #expect(
      parser.parse(elementValue: axis(usage: 0x35, integer: 255)) == [.rightTriggerChanged(1)]
    )
    #expect(
      parser.parse(elementValue: value(page: 0x01, usage: 0x39, minimum: 0, maximum: 7, integer: 3))
        == [.dpadChanged(.southEast)]
    )
    #expect(
      parser.parse(elementValue: value(page: 0x01, usage: 0x39, minimum: 0, maximum: 7, integer: 8))
        == [.dpadChanged(.neutral)]
    )
