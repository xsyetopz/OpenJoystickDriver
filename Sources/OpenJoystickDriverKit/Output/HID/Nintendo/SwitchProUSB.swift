...
/// 64-byte HID lengths. Bluetooth UART is out of scope.
public enum SwitchProUSBHIDDescriptor {
  ...
  /// USB HID report descriptor for `057E:2009` (Joystick, report `0x30`).
...
enum SwitchProHIDBits {
  ...
  static func buttons(_ state: VirtualGamepadState) -> UInt32 {
    ...
    if state.effectiveRightTrigger > 0 { bits |= 0x0000_0080 }
    if state.effectiveLeftTrigger > 0 { bits |= 0x0080_0000 }
  ...
  static func factoryStickCalibration() -> [UInt8] {
    pack12(factoryExtent, factoryExtent, factoryCenter, factoryCenter, factoryExtent, factoryExtent)
      + pack12(
        factoryCenter,
        factoryCenter,
        factoryExtent,
