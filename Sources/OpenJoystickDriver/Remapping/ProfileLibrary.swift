...
enum RemappingProfileLibraryError: Error, Equatable, LocalizedError, Sendable {
  ...
  var errorDescription: String? {
    switch self {
    case .corruptLibrary:
      "The remapping profile library is corrupt."
    case .duplicateName(let name):
      "A remapping profile named \(name) already exists."
    ...
    case .profileAlreadyExists(let id):
      "The remapping profile \(id.uuidString) already exists."
    case .profileNotFound(let id):
      "The remapping profile \(id.uuidString) does not exist."
    case .profileUpdateConflict(let id):
      "The remapping profile \(id.uuidString) changed since it was read."
    case .unreadableLibrary:
      "The remapping profile library could not be read."
    case .unsupportedLibraryVersion(let version):
      "Unsupported remapping profile library version: \(version)."
    case .unwritableLibrary:
      "The remapping profile library could not be written."
    }
  }
}
...
actor RemappingProfileLibrary {
  ...
  init(fileURL: URL = RemappingProfileLibrary.defaultFileURL) {
  ...
  static var defaultFileURL: URL {
    ...
    return directory
      .appendingPathComponent("OpenJoystickDriver", isDirectory: true)
  ...
  func snapshot() throws -> RemappingProfileLibrarySnapshot {
    ...
    return RemappingProfileLibrarySnapshot(
      ...
      activeProfiles: loaded.activeProfiles.map {
        RemappingActiveProfileSelection(model: $0.model, profileID: $0.profileID)
  ...
  @discardableResult func activate(profileID: UUID) throws -> RemappingProfileMutationImpact {
    ...
    proposed.activeProfiles.append(
      RemappingPersistedActiveProfile(model: model, profileID: profile.id)
    )
    try replace(with: proposed)
    return RemappingProfileMutationImpact(modelsNeedingRefresh: [model])
  }

  @discardableResult func deactivate(vendorID: UInt16, productID: UInt16) throws
    -> RemappingProfileMutationImpact
  {
    var proposed = try loadIfNeeded()
  ...
  func activeProfile(vendorID: UInt16, productID: UInt16) throws -> RemappingProfile? {
    let loaded = try loadIfNeeded()
    let model = RemappingProfileModel(vendorID: vendorID, productID: productID)
    guard let active = loaded.activeProfiles.first(where: { $0.model == model }) else { return nil }
    return loaded.profiles.first { $0.id == active.profileID }
  }
  ...
  private func replace(with proposed: RemappingProfileLibraryState) throws {
    ...
    let data: Data
    do {
      data = try JSONEncoder().encode(proposed)
    } catch {
      throw RemappingProfileLibraryError.unwritableLibrary
    }
    guard data.count <= Self.maximumEncodedBytes else {
  ...
  private func validate(_ profile: RemappingProfile, in peers: [RemappingProfile]) throws {
    do {
      ...
      throw RemappingProfileLibraryError.invalidProfile(.encodingFailed)
    }
  ...
  private func validate(_ library: RemappingProfileLibraryState) throws {
    ...
    let activeModels = Set(library.activeProfiles.map(\.model))
    guard activeModels.count == library.activeProfiles.count,
      library.activeProfiles.allSatisfy({ active in
  ...
  private static func activeProfileOrder(_ lhs: RemappingActiveProfileSelection,
    _ rhs: RemappingActiveProfileSelection) -> Bool
  {
    if lhs.model.vendorID != rhs.model.vendorID {
      return lhs.model.vendorID < rhs.model.vendorID
    }
