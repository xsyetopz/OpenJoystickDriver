...
struct MenuBarPopoverView: View {
  ...
  var body: some View {
    ...
    }.frame(width: 440).alert(isPresented: $showUninstallConfirm) {
      Alert(
  ...
  var headerRow: some View {
    HStack(alignment: .center, spacing: 12) {
      VStack(alignment: .leading, spacing: 2) {
        Text(L10n.string("app.name")).font(.system(size: 17, weight: .semibold))
      }
      Spacer()
  ...
  var readinessCard: some View {
    ...
    let summary: String = {
      ...
      case .restarting: return L10n.string("readiness.daemonRestarting")
      case .crashLooping: return L10n.string("readiness.daemonCrashLooping")
      case .missing: return L10n.string("readiness.installDaemon")
      case .stopped: return L10n.string("readiness.startDaemon")
      case .runningDisconnected: return L10n.string("readiness.daemonDisconnected")
      case .runningConnected, .unknown: break
      }
    ...
    return OJDCard {
      HStack(alignment: .top, spacing: 12) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          ...
          Text(summary).font(.caption).foregroundColor(.secondary).fixedSize(
            horizontal: false,
            vertical: true
          )
        }
        Spacer()
        Text(model.daemonStatusLabel).font(.caption.weight(.semibold)).padding(.horizontal, 8)
          .padding(.vertical, 4).foregroundColor(ready ? .green : .secondary).background(
      ...
      HStack(spacing: 10) {
        ...
        MetricChip(
          ...
          value: permissionsReady
            ? L10n.string("access.allowed") : L10n.string("access.needsAccess")
        )
      }.padding(.top, 4)

      if !ready { readinessAction }
  ...
  @ViewBuilder var readinessAction: some View {
      ...
      VStack(alignment: .leading, spacing: 10) {
        HStack(alignment: .top, spacing: 10) {
          VStack(alignment: .leading, spacing: 3) {
            Text(L10n.string("profile.description")).font(.caption).foregroundColor(.secondary)
        ...
        HStack(spacing: 10) {
          Text(L10n.string("profile.identity")).font(.caption).foregroundColor(.secondary)
          Picker(
            ...
            Text(L10n.string("profile.appleGameController")).tag(
              CompatibilityIdentity.appleGameController.rawValue
            )
          ...
          }.frame(maxWidth: .infinity).disabled(!model.daemonConnected || !compatSelected)
        }

        if !compatSelected {
          Text(L10n.string("profile.switchToCompatibility")).font(.caption).foregroundColor(
            .secondary
          )
        }
      }
