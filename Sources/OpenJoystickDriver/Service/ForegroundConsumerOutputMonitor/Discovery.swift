...
extension ForegroundConsumerOutputMonitor {
  /// Runs one production consumer-discovery scan inside an explicit autorelease pool.
  ///
  /// Used by the application service's isolated allocation probe; it does not start RPC, USB,
  /// virtual output, or the periodic monitor.
  static func diagnosticConsumerClientCount() -> Int {
    autoreleasepool { consumerClientSamples().count }
  }

  static func consumerClientSamples() -> [ForegroundConsumerClientSample] {
  ...
  private static func isOJDVirtualDevice(service: io_service_t, device: IOHIDDevice) -> Bool {
    ...
    let isDriverKit = DriverKitRelayIdentity.matches(
      ...
      transport: stringProperty(kIOHIDTransportKey as String),
      vendorID: UInt32(truncatingIfNeeded: intProperty(kIOHIDVendorIDKey as String)),
      productID: UInt32(truncatingIfNeeded: intProperty(kIOHIDProductIDKey as String)),
      versionNumber: UInt32(truncatingIfNeeded: intProperty(kIOHIDVersionNumberKey as String)),
      locationID: rawLocation,
      manufacturer: stringProperty(kIOHIDManufacturerKey as String),
      product: stringProperty(kIOHIDProductKey as String),
      serialNumber: serial,
      primaryUsagePage: UInt32(
        truncatingIfNeeded: intProperty(kIOHIDPrimaryUsagePageKey as String)
      ),
      primaryUsage: UInt32(truncatingIfNeeded: intProperty(kIOHIDPrimaryUsageKey as String))
    )
    if isDriverKit { return true }
  ...
  static func bundleLabelList(_ bundleRoots: Set<String>) -> String {
  ...
  static func bundleRootPath(for application: NSRunningApplication?) -> String? {
  ...
  private static func isIgnoredConsumerBundleRoot(_ bundleRoot: String) -> Bool {
    ...
    return name == "OpenJoystickDriver.app"
  }
}
